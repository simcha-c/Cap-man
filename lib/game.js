...
class Game {
  constructor(sprite) {
    ...
    this.pacman = new Pacman(c, 300, 507.5, this.board.tile, sprite);
    this.ghosts = [];
    this.ctx = c;
    this.lives = 3;
    this.gameOver = false;
  ...
  createGhosts() {
    ...
    for (var i = 0; i < colors.length; i++) {
      let newGhost = new Ghost(this.ctx, (300 + (i*2)), 297.5, colors[i], this.level, this.board.tile, this.sprite);
