...
class Game {
  ...
  drawLife(num) {
    this.ctx.beginPath();
    this.ctx.arc(315 + (28 * num), 695, 12, 0.2 * Math.PI, 1.8 * Math.PI);
    this.ctx.lineTo(315 + (28 * num), 695);
    this.ctx.closePath();
  ...
  animate(firstAnimation) {
    ...
    this.ctx.fillText(`LIVES:`, 250, 702);
...
const game = () => {
  ...
  sprite.onload = () => {
    ...
    c.fillStyle = "blue";
    // c.font = '18px Arial';
    c.font = "50px OrbitronBlack";
    c.fillText("Welcome to", 130, 200);
    c.font = "65px OrbitronBlack";
    c.fillText("CAPMAN", 130, 280);
    ...
    c.fillStyle = "white";
    c.fillText("PRESS ENTER TO START", 150, 600);
  });
...
const drawGhost = (canvas, c, x, y, color) => {
  let s = 60,
    ...
    c.beginPath();
    c.arc(500, 500, 10, 0, 2 * Math.PI, false);
